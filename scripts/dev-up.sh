...
# the admin static server, the MQTT client static server, and a free shell (with
# service/.venv activated). Works from any terminal app (tmux owns the panes, not the
# surrounding app).
...
echo "Started tmux session '$SESSION': docker | admin | client-mqtt | shell"
