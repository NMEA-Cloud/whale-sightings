...
# Geolocation (this client doesn't use it, but its sibling clients do — see the
# "use current location" radius filter) requires a secure context. Reuses the same shared
# step-ca-issued cert every other TLS-serving container in this project already mounts
# (certs/localhost.pem/-key.pem) — its SANs already cover whatever hostname this is opened
# with (see scripts/setup-tls.sh), so no separate cert is needed here.
